...
class _RasterizeGaussians(torch.autograd.Function):
        ...
        def forward(
            ...
            if raster_settings.debug:
                ...
                try:
                    num_rendered, color, flow, depth, T, radii, geomBuffer, binningBuffer, imgBuffer, covs_com, out_means3D, radii_static, color_4d, color_3d, invdepth = _C.rasterize_gaussians(*args)
            ...
            ctx.save_for_backward(
                ...
                means3D_static, means2D_static, shs_static, opacities_static, scales_static, rotations_static,
                geomBuffer, binningBuffer, imgBuffer
            )
            return color, radii, depth, 1-T, flow, covs_com, radii_static, color_4d, color_3d, invdepth

        @staticmethod
        def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_alpha, grad_flow, grad_covs_com, grad_radii_static, grad_color_4d, grad_color_3d, grad_invdepth):
            ...
            if raster_settings.debug:
                ...
                try:
                    (
                        ...
                        grad_flows, grad_ts, grad_scales, grad_scales_t, grad_rotations, grad_rotations_r,
                        grad_means2D_static, grad_opacities_static, grad_shs_static, grad_scales_static, grad_rotations_static
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(
        ...
        if means3D_static is None:
            means3D_static = torch.Tensor([])
        if means2D_static is None:
            means2D_static = torch.Tensor([])
        if shs_static is None:
            shs_static = torch.Tensor([])
        if opacities_static is None:
            opacities_static = torch.Tensor([])
        if scales_static is None:
            scales_static = torch.Tensor([])
        if rotations_static is None:
            rotations_static = torch.Tensor([])
