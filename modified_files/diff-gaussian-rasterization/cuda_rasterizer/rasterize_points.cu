...
RasterizeGaussiansCUDA(
    ...
    const torch::Tensor& means2D_static, // Unused but kept for compatibility
    const torch::Tensor& shs_static,
    ...
    const bool debug)
{
    ...
    if (means3D_static.ndimension() != 2 || means3D_static.size(1) != 3) {
        AT_ERROR("means3D_static must have dimensions (num_points_static, 3)");
    }
    ...
    torch::Tensor out_T = torch::full({H, W}, 0.0, float_opts);
    torch::Tensor radii = torch::full({P}, 0, int_opts);
    torch::Tensor radii_static = torch::full({P_static}, 0, int_opts);
    torch::Tensor color_4d = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts);
    torch::Tensor color_3d = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts);
    torch::Tensor invdepth = torch::full({H, W}, 0.0, float_opts);
    ...
    int rendered = 0;
    if (P + P_static != 0) {
        int M = sh.size(0) ? sh.size(1) : 0;
        int M_static = shs_static.size(0) ? shs_static.size(1) : 0;

        rendered = CudaRasterizer::Rasterizer::forward(
            geomFunc,
            binningFunc,
            imgFunc,
            P, P_static, degree, degree_t, std::max(M, M_static),
            background.contiguous().data<float>(),
            W, H,
            means3D.contiguous().data<float>(),
            means3D_static.contiguous().data<float>(),
            out_means3D.contiguous().data<float>(),
            sh.contiguous().data_ptr<float>(),
            shs_static.contiguous().data_ptr<float>(),
            colors.contiguous().data<float>(),
            flows.contiguous().data<float>(),
            opacity.contiguous().data<float>(),
            opacities_static.contiguous().data<float>(),
            ...
            radii_static.contiguous().data<int>(),
            color_4d.contiguous().data<float>(),
            color_3d.contiguous().data<float>(),
            invdepth.contiguous().data<float>(),
            debug);
    }

    char* geo_ptr = reinterpret_cast<char*>(geomBuffer.contiguous().data_ptr());
    CudaRasterizer::GeometryState geoState = CudaRasterizer::GeometryState::fromChunk(geo_ptr, P + P_static);

    torch::Tensor covs3D_com = torch::from_blob(geoState.cov3D, {P + P_static, 6}, float_opts);
    return std::make_tuple(
        rendered, out_color, out_flow, out_depth, out_T, radii, geomBuffer, binningBuffer, imgBuffer,
        covs3D_com, out_means3D, radii_static, color_4d, color_3d, invdepth
    );
}
